...
async function analyzeGitHubRepo(owner: string, repo: string, token?: string, maxPages: number = MAX_PAGES): Promise<ScanResult> {
  ...
  function processPage(commits: any[]) {
    for (const commit of commits) {
      ...
      const tool = detectAiTool(msg, authorEmail)
      if (tool !== 'Human') {
        aiCommits++
...
function detectAiTool(msg: string, authorEmail?: string): string {
  const email = (authorEmail || '').toLowerCase()
  if (msg.includes('co-authored-by: claude') || msg.includes('noreply@anthropic.com') || msg.includes('generated with claude code') || msg.includes('claude.ai/code/') || msg.includes('claude.ai/chat/') || email === 'noreply@anthropic.com') return 'Claude Code'
  if (msg.includes('co-authored-by: cursor')) return 'Cursor'
  if (msg.includes('co-authored-by: aider') || msg.includes('noreply@aider.chat') || msg.includes('aider: ') || email === 'noreply@aider.chat') return 'Aider'
